...
namespace MyGame
{
    ...
    public class BasicCameraController : SyncScript
    {
        ...
        public float FilterAlpha { get; set; } = 0.5f;
        ...
        private void ProcessInput()
        {
            ...
            //move towards 0
            yaw = MathUtil.Lerp(yaw, 0f, FilterAlpha);
            pitch = MathUtil.Lerp(pitch, 0f, FilterAlpha);
            orbitYaw = MathUtil.Lerp(orbitYaw, 0f, FilterAlpha);
            orbitPitch = MathUtil.Lerp(orbitPitch, 0f, FilterAlpha);
            ...
            // Mouse movement and gestures
            {
                ...
                if (Input.HasMouse)
                {
                    ...
                    if (Input.IsMouseButtonDown(MouseButton.Right))
                    {
                        ...
                        orbitPitch -= Input.MouseDelta.X * MouseRotationSpeed.X;
                        orbitYaw -= Input.MouseDelta.Y * MouseRotationSpeed.Y;
                    }
                    else
                    {
                ...
                foreach (var gestureEvent in Input.GestureEvents)
                {
                    switch (gestureEvent.Type)
                    {
                        ...
                        case GestureType.Drag:
                            ...
                            orbitYaw = -dragDistance.X * TouchRotationSpeed.X;
                            orbitPitch = -dragDistance.Y * TouchRotationSpeed.Y;
                            break;
