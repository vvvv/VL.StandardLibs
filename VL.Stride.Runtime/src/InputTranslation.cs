...
namespace VL.Stride
{
    ...
    {
        public static IObservable<INotification> GetNotifications(this InputManager inputManager, IInputSource inputSource, object sender = null)
        {
            return Observable.Create<INotification>(observer =>
            {
                var subscription = new CompositeDisposable();

                var mouseButtonListener = NewMouseButtonListener(observer, inputSource, subscription);
                inputManager.AddListener(mouseButtonListener);
                var mouseWheelListener = NewMouseWheelListener(observer, inputSource);
                inputManager.AddListener(mouseWheelListener);
                var pointerListener = NewPointerListener(observer, inputSource);
                inputManager.AddListener(pointerListener);
                var keyListener = NewKeyListener(observer, inputSource);
                inputManager.AddListener(keyListener);
                var textListener = NewTextInputListener(observer, inputSource);
            ...
            IInputEventListener NewPointerListener(IObserver<INotification> observer, IInputSource inputSource)
            {
                return new AnonymousEventListener<PointerEvent>(e =>
                {
                    ...
                    switch (e.EventType)
                    {
                        case PointerEventType.Pressed:
                            ...
                            {
                                var n = new TouchNotification(TouchNotificationKind.TouchDown, e.AbsolutePosition, pointerDevice.SurfaceSize, e.PointerId, primary: e.PointerId == 0, default, default, modifiers, sender);
                                observer.OnNext(n);
                                if (!n.Handled)
                                    observer.OnNext(new MouseDownNotification(e.Position * pointerDevice.SurfaceSize, pointerDevice.SurfaceSize, MouseButtons.Left, modifiers, sender));
                            }
                            break;
                        case PointerEventType.Moved:
                            if (e.Device is IMouseDevice)
                                observer.OnNext(new MouseMoveNotification(e.AbsolutePosition, pointerDevice.SurfaceSize, modifiers, sender));
                            else
                            {
                                var n = new TouchNotification(TouchNotificationKind.TouchMove, e.AbsolutePosition, pointerDevice.SurfaceSize, e.PointerId, primary: e.PointerId == 0, default, default, modifiers, sender);
                                observer.OnNext(n);
                                if (!n.Handled)
                                    observer.OnNext(new MouseMoveNotification(e.Position * pointerDevice.SurfaceSize, pointerDevice.SurfaceSize, modifiers, sender));
                            }
                            break;
                        case PointerEventType.Released:
                            ...
                            if (e.Device is not IMouseDevice)
                            {
                                var n = new TouchNotification(TouchNotificationKind.TouchUp, e.AbsolutePosition, pointerDevice.SurfaceSize, e.PointerId, primary: e.PointerId == 0, default, default, modifiers, sender);
                                observer.OnNext(n);
                                if (!n.Handled)
                                    observer.OnNext(new MouseUpNotification(e.Position * pointerDevice.SurfaceSize, pointerDevice.SurfaceSize, MouseButtons.Left, modifiers, sender));
                            }
                            break;
            ...
            IInputEventListener NewMouseButtonListener(IObserver<INotification> observer, IInputSource inputSource, CompositeDisposable subscription)
            {
                ...
                mouse.Notifications.Subscribe(n => observer.OnNext(n)).DisposeBy(subscription);
                return new AnonymousEventListener<MouseButtonEvent>(e =>
                {
                    ...
                    var modifiers = GetModifierKeys(inputSource);
                    if (e.IsDown)
                        eventStream.OnNext(new MouseDownNotification(mouseDevice.Position * mouseDevice.SurfaceSize, mouseDevice.SurfaceSize, ToWinFormsButton(e.Button), modifiers, sender));
                    else
                        eventStream.OnNext(new MouseUpNotification(mouseDevice.Position * mouseDevice.SurfaceSize, mouseDevice.SurfaceSize, ToWinFormsButton(e.Button), modifiers, sender));
                });
            }

            IInputEventListener NewMouseWheelListener(IObserver<INotification> observer, IInputSource inputSource)
            {
                return new AnonymousEventListener<MouseWheelEvent>(e =>
                {
                    ...
                    var mouseDevice = e.Device as IMouseDevice;
                    var modifiers = GetModifierKeys(inputSource);
                    observer.OnNext(new MouseWheelNotification(mouseDevice.Position * mouseDevice.SurfaceSize, mouseDevice.SurfaceSize, (int)e.WheelDelta * 120, modifiers, sender));
                });
            }

            IInputEventListener NewKeyListener(IObserver<INotification> observer, IInputSource inputSource)
            {
                return new AnonymousEventListener<KeyEvent>(e =>
                {
                    ...
                    if (e.IsDown)
                        observer.OnNext(new KeyDownNotification(key, modifierKeys, sender));
                    else
                        observer.OnNext(new KeyUpNotification(key, modifierKeys, sender));
                });
            }

            IInputEventListener NewTextInputListener(IObserver<INotification> observer, IInputSource inputSource)
            {
                return new AnonymousEventListener<TextInputEvent>(e =>
                {
                    ...
                    if (e.Type == TextInputEventType.Input)
                    {
                        ...
                        foreach (var c in e.Text)
                            observer.OnNext(new KeyPressNotification(c, modifiers, sender));
                    }
                });
            }
        }
