    ...
    static partial class GraphicsNodes
    {
        public static IEnumerable<IVLNodeDescription> GetNodeDescriptions(IVLNodeDescriptionFactory factory)
        {
            ...
            yield return factory.NewDescriptionNode(graphicsCategory, BlendStates.Default)
               ...
               .AddCachedInput("RenderTarget0", x => x.v.RenderTargets[0], (x, v) => x.v.RenderTargets[0] = v)
               .AddCachedInput("RenderTarget1", x => x.v.RenderTargets[1], (x, v) => x.v.RenderTargets[1] = v)
               .AddCachedInput("RenderTarget2", x => x.v.RenderTargets[2], (x, v) => x.v.RenderTargets[2] = v)
               .AddCachedInput("RenderTarget3", x => x.v.RenderTargets[3], (x, v) => x.v.RenderTargets[3] = v)
               .AddCachedInput("RenderTarget4", x => x.v.RenderTargets[4], (x, v) => x.v.RenderTargets[4] = v)
               .AddCachedInput("RenderTarget5", x => x.v.RenderTargets[5], (x, v) => x.v.RenderTargets[5] = v)
               .AddCachedInput("RenderTarget6", x => x.v.RenderTargets[6], (x, v) => x.v.RenderTargets[6] = v)
               .AddCachedInput("RenderTarget7", x => x.v.RenderTargets[7], (x, v) => x.v.RenderTargets[7] = v)
               .AddStateOutput();
            ...
            yield return factory.NewDescriptionNode(graphicsCategory, TextureDescription.New2D(512, 512, PixelFormat.R8G8B8A8_UNorm_SRgb))
                ...
                .AddCachedInput("MipLevels", x => x.v.MipLevelCount, (x, v) => x.v.MipLevelCount = v, 1)
